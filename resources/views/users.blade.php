        ...
        <table class="table table-dark table-hover text-center">
            ...
            <tr>
                @foreach ($users as $user)
                    ...
                    <td>
                        <button class="btn btn-outline-warning btn-icon" data-bs-toggle="modal" data-bs-target="#modalCreateUserForm" id="openCreateUserForm"><i class="bi bi-plus"></i></button>
                        <button class="btn btn-outline-warning btn-icon" data-bs-toggle="modal" data-bs-target="#modalUpdateUserForm_{{ $user->id }}" id="openUpdateUserForm_{{ $user->id }}"><i class="bi bi-list"></i></button>
                        <form action="{{ route('users.destroy', $user) }}" method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-outline-warning btn-icon"><i class="bi bi-x"></i></button>
                        </form>
                        <div class="modal fade" id="modalUpdateUserForm_{{ $user->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                             aria-hidden="true">
                                    ...
                                    <div class="modal-body p-5 pt-0">
                                        <form action="{{ route('users.update', $user) }}" method="post">
                                            ...
                                            <div class="form-floating mb-4">
                                                <input type="text" id="update_user_name" name="update_user_name" class="form-control rounded-3 text-white {{$errors->has('update_user_name') ? 'is-invalid' : ''}}" placeholder="" value="{{old('update_user_name', $user->name)}}" required>
                                                <label for="update_user_name" class="text-white">Имя пользователя</label>
                                                @error('update_user_name')
                                                <small class="text-danger">{{$message}}</small>
                                                @enderror
                                            </div>
                                            <div class="form-floating mb-4">
                                                <input type="email" id="update_user_email" name="update_user_email" class="form-control rounded-3 text-white {{$errors->has('update_user_email') ? 'is-invalid' : ''}}" placeholder="" value="{{old('update_user_email', $user->email)}}" required>
                                            ...
                                            <div class="form-floating mb-4">
                                                <select id="update_user_address" name="update_user_address" class="form-control text-white {{$errors->has('update_user_address') ? 'is-invalid' : ''}}" aria-label="Точка работы" required>
                                                    ...
                                                    <option value="Пр.Молодежный, 2Б" {{ old('update_user_address', $user->address) == 'Пр.Молодежный, 2Б' ? 'selected' : '' }}>Пр. Ленина, 28</option>
                        ...
                        @if($errors->has('update_user_name') || $errors->has('update_user_email') || $errors->has('update_user_address') || $errors->has('update_user_level'))
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                document.getElementById('openUpdateUserForm_{{ $user->id }}').click();
                            });
                        </script>
                        @endif
                ...
                <div class="modal-body p-5 pt-0">
                    <form action="{{route('users.store')}}" method="post">
                        ...
                        <div class="form-floating mb-4">
                            ...
                            <label for="forced_name" class="text-white">Имя пользователя</label>
                            @error('forced_name')
                            <small class="text-danger">{{$message}}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-floating mb-4">
                            <select id="forced_address" name="forced_address" class="form-control text-white {{$errors->has('forced_address') ? 'is-invalid' : ''}}" aria-label="Точка работы" required>
                                ...
                                <option value="Пр.Молодежный, 2Б" {{ old('forced_address') == 'Пр.Молодежный, 2Б' ? 'selected' : '' }}>Пр. Ленина, 28</option>
